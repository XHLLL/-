...
void Key_Init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 |GPIO_Pin_1 | GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_13;
...
uint8_t Key_GetNum(void)
{
    ...
    if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_0) == 0)          //读PB0输入寄存器的状态，如果为0，则代表按键1按下
    {
        Delay_ms(20);                                            //延时消抖
    ...
    if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_10) == 0)            //读PB10输入寄存器的状态，如果为0，则代表按键3按下
    {
        Delay_ms(20);                                            //延时消抖
    ...
    if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_13) == 0)            //读PB13输入寄存器的状态，如果为0，则代表按键5按下
    {
        Delay_ms(20);                                            //延时消抖
