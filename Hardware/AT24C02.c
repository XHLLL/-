...
{
    MyI2C_Start();
    MyI2C_SendByte(AT24C02_ADDRESS_READ);
    MyI2C_ReceiveAck();
...
u8 AT24C02_ReadByte(u8 WordAddress)
{
    ...
    MyI2C_SendByte(WordAddress);
    MyI2C_ReceiveAck();
    MyI2C_Start();  // 发送开始信号，不要发送停止信号
    MyI2C_SendByte(AT24C02_ADDRESS_WRITE);  // 发送设备地址，写入模式
    MyI2C_ReceiveAck();
    Data=MyI2C_ReceiveByte();
    MyI2C_SendAck(1);  // 发送非应答位，继续读取下一个字节
    MyI2C_Stop();  // 停止信号放在最后一次读取完数据后发送
    return Data;
}

/**
  * @brief  AT24C02写入一个浮点数
  * @param  WordAddr 要写入浮点数的地址
...
/**
  * @brief  AT24C02读取一个浮点数
  * @param  ReadAddr 要读出浮点数的地址
  * @retval 读出的数据
  */
void AT24C02_Read_Float(u16 ReadAddr, float *pBuffer, u16 NumToRead)
{
    ...
    for (i = 0; i < 4; i++)
    {
        fb.b[i] = AT24C02_ReadByte(ReadAddr);
        ReadAddr++;
        Delay_ms(5);
    }
    *pBuffer = fb.a;
...
void AT24C02_Erase(uint16_t addr, uint16_t size)
{
    ...
    for (i = 0; i < size; i++)
    {
        AT24C02_WriteByte(addr + i, 0xFF); // 擦除数据，写入0xFF
        Delay_ms(5);                       // 延迟一段时间等待擦除完成
    }
}
